...
cmd-gc_mode-help =
    ...
    Если аргумент не был предоставлен, вернется текущий режим задержки GC.
...
# Примечание: команда flushcookies взята из Robust.Client.WebView, ее нет в коде основного движка.
...
cmd-togglehardfov-desc = Включает жесткое поле зрения клиента. (для отладки space-station-14#2353)
...
cmd-cldbglyr-desc = Переключение слоев отладки поле зрения и освещения.
cmd-cldbglyr-help =
...
cmd-tpto-parse-error = Не удается распознать сущность или игрока: { $str }
...
# Spin command
cmd-spin-desc = Заставляет сущность вращаться. Сущность по умолчанию является надклассом прикрепленного игрока.
