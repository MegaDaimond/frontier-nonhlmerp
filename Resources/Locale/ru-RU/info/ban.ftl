...
cmd-ban_exemption_get-desc = Показать исключения банов для определенного игрока.
...
cmd-ban_exemption_get-show = Пользователь исключен из банов со следующими флагами: { $flags }.
# Ban panel
...
ban-panel-invalid-ip = Не удается спарсить IP адрес. Попробуйте еще раз
...
server-ban-string-no-pii = { $admin } установил серверный бан { $severity } тяжести, который истечет { $expires } у { $name } с причиной: { $reason }
