# other
construction-graph-tag-nf-butter-stick = пачка масла
construction-graph-tag-nf-small-thruster-machine-circuitboard = плата малого двигателя
construction-graph-tag-shell-shotgun = дробовой патрон
construction-graph-tag-flare = фальшфейер
construction-graph-tag-medal = медаль
construction-graph-tag-whoopie-cushion = подушка-пердушка
construction-graph-tag-material-hide-rosy-mothroach = шкура розового мотылька
construction-graph-tag-freezer-electronics-circuit-board = электронная плата морозильника
construction-graph-tag-toolbox = ящик для инструментов
construction-graph-tag-ore = руда
construction-graph-tag-crystal-orange = оранжевый кристаллический осколок
construction-graph-tag-mothroach-hide = шкура мотылька
construction-graph-tag-syringe = шприц
