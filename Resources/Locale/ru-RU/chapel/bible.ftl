...
necro-heal-fail-self = Вы ударяете { $target } с помощью { $bible }, но удар отдается печальным стуком, не сумев поразить { $target }.
necro-heal-fail-others = { CAPITALIZE($user) } ударяет { $target } с помощью { $bible }, но удар отдается печальным стуком, не сумев поразить { $target }.
