ent-LPPBeachVendingMachine = Beach machine
    .desc = a machine that decorates the appearance of customers and makes them beautiful!
    .suffix = Unlocked
ent-LPPCargodrobeVard = Cargo vard machine
    .desc = Fashionable suits for Varden!
    .suffix = Unlocked
ent-LPPErpMachine = Erp machine
    .desc = Fashionable suits for Varden!
    .suffix = Unlocked
ent-LPPBrasMat = Bras machine
    .desc = a machine that decorates the appearance of customers and makes them beautiful!
    .suffix = Unlocked
ent-LPPWithMedal = With medal machine
    .desc = For when you need seeds fast. Hands down the best seed selection on the station!
    .suffix = Unlocked
ent-LPPNascaMat = Nasca machine
    .desc = a machine that decorates the appearance of customers and makes them beautiful!
    .suffix = Unlocked
ent-LPPPillowVendingMachine = Pillow vending machine
    .desc = For when you need seeds fast. Hands down the best seed selection on the station!
    .suffix = Unlocked
ent-LPPUnderpantsMat = Underpants machine
    .desc = a machine that decorates the appearance of customers and makes them beautiful!
    .suffix = Unlocked
ent-LPPWithSoftAnimals = With soft animals machine
    .desc = For when you need seeds fast. Hands down the best seed selection on the station!
    .suffix = Unlocked
ent-LPPVendingMachineBlackMarketEquipement = BlackMarketvend
    .desc = Contraband across multiple sectors!
ent-LPPVendingMachineBlackMarket = BlackMarketvend
    .desc = Clothing across multiple sectors!
ent-LPPVendingMachineBlackMarketArmor = BlackMarketvendArmor
    .desc = Clothing across multiple sectors!
